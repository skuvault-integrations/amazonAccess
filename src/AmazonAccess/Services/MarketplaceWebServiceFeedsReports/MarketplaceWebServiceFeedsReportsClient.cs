...
using AmazonAccess.Services.MarketplaceWebServiceFeedsReports.Attributes;
...
namespace AmazonAccess.Services.MarketplaceWebServiceFeedsReports
{
    ...
    */
	...
	public class MarketplaceWebServiceFeedsReportsClient: IMarketplaceWebServiceFeeds
	{
		...
		private MarketplaceWebServiceFeedsReportsConfig config = null;
		...
		public MarketplaceWebServiceFeedsReportsClient(
			String awsAccessKeyId,
			String awsSecretAccessKey,
			MarketplaceWebServiceFeedsReportsConfig config )
		{
			...
			{
				throw new MarketplaceWebServiceFeedsReportsException( "Missing required value: User-Agent." );
			}
		...
		public MarketplaceWebServiceFeedsReportsClient(
			...
			: this(
				...
				applicationName,
				applicationVersion,
				new MarketplaceWebServiceFeedsReportsConfig() )
		{
		}
		...
		private T Invoke< T, K >( IDictionary< String, String > parameters, K clazz )
		{
			...
			{
				throw new MarketplaceWebServiceFeedsReportsException( new ArgumentException(
			...
			do
			{
				...
				catch( WebException we )
				{
					using( var httpErrorResponse = ( HttpWebResponse )we.Response )
					{
						...
						{
							throw new MarketplaceWebServiceFeedsReportsException( we );
						}
					...
					try
					{
						...
						throw new MarketplaceWebServiceFeedsReportsException(
							error.Message,
							statusCode,
							error.Code,
							error.Type,
					...
					catch( Exception e )
					{
						...
						if( e is MarketplaceWebServiceFeedsReportsException )
						{
							throw;
						}
		...
		private MarketplaceWebServiceFeedsReportsException ReportAnyErrors( String responseBody, HttpStatusCode status, Exception e, ResponseHeaderMetadata rhm )
		{

			MarketplaceWebServiceFeedsReportsException ex = null;
			...
			if( responseBody != null && responseBody.StartsWith( "<" ) )
			{
				...
				if( errorMatcherOne.Success )
				{
					...
					ex = new MarketplaceWebServiceFeedsReportsException( message, status, code, "Unknown", requestId, responseBody, rhm );
				...
				{
					ex = new MarketplaceWebServiceFeedsReportsException( "Internal Error", status, rhm );
				}
			}
			else
			{
		...
		private void PauseOnRetry( int retries )
		{
			...
			AmazonLogger.Log.Trace( "[amazon] Feeds. Pause on retry. Seller {0}", this._sellerId );
